...
//			NULL, NULL);
    handle->sws_ctx = sws_getContext(handle->params.inwidth,
            handle->params.inheight, handle->inavfmt, handle->params.outwidth,
            handle->params.outheight, handle->outavfmt, SWS_POINT, NULL,
            NULL, NULL);
	...
	// alloc buffers
	handle->src_frame = avcodec_alloc_frame();
	if (!handle->src_frame)
	{
	...
	handle->dst_frame =  avcodec_alloc_frame();
	if (!handle->dst_frame)
	{
	...
	err4: avcodec_free_frame(&handle->dst_frame);
	err3: av_free(handle->src_buffer);
	err2: avcodec_free_frame(&handle->src_frame);
